...
public class SQLSafeQueryExecutor {
    ...
    public ResultSet selectFromEatenMealsTable(String username) throws SQLException {
        ...
        CachedRowSet cachedRowSet;
        try (Statement statement = connection.createStatement()) {
            resultSet = statement.executeQuery(String.format("SELECT MealCompositions.mealName AS mealName,\n" + "       Ingredients.ingredientName AS ingredientName,\n" + "       MealCompositions.quantity * ingredientsRatio AS quantity,\n" + "       quantityType,\n" + "       ingredientsRatio,\n" + "       gramsPerQuantity,\n" + "       mealNumber,\n" + "       price * ( gramsPerQuantity * MealCompositions.quantity * ingredientsRatio/ 100) AS price,\n" + "       complexCarbs * ( gramsPerQuantity * MealCompositions.quantity * ingredientsRatio/ 100) AS complexCarbs,\n" + "       simpleCarbs * ( gramsPerQuantity * MealCompositions.quantity * ingredientsRatio/ 100) AS simpleCarbs,\n" + "       fibers * ( gramsPerQuantity * MealCompositions.quantity * ingredientsRatio/ 100) AS fibers,\n" + "       saturatedFats * ( gramsPerQuantity * MealCompositions.quantity * ingredientsRatio/ 100) AS saturatedFats,\n" + "       unSaturatedFats * ( gramsPerQuantity * MealCompositions.quantity *ingredientsRatio/ 100) AS unSaturatedFats,\n" + "       transFats * ( gramsPerQuantity * MealCompositions.quantity * ingredientsRatio/ 100) AS transFats,\n" + "       proteins * ( gramsPerQuantity * MealCompositions.quantity * ingredientsRatio/ 100) AS proteins,\n" + "       alcohol * ( gramsPerQuantity * MealCompositions.quantity * ingredientsRatio/ 100) AS alcohol\n" + "FROM Meals LEFT JOIN MealCompositions ON Meals.mealName = MealCompositions.mealName\n" + "LEFT JOIN Ingredients ON MealCompositions.ingredientName = Ingredients.ingredientName\n" + "JOIN EatenMeals ON MealCompositions.mealName = EatenMeals.mealName\n" + "WHERE username = '%s'\n" + "ORDER BY mealNumber;", username));
            cachedRowSet = cache(resultSet);
        }
        return cachedRowSet;
    }
