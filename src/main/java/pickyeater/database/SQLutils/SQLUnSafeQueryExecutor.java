...
public class SQLUnSafeQueryExecutor {
    public void insertIntoIngredientsTable(Ingredient ingredient) throws SQLException {
        ...
        String query = "INSERT INTO Ingredients " + "(ingredientName,price,quantity,quantityType,gramsPerQuantity,complexCarbs,simpleCarbs," + "fibers,saturatedFats,unsaturatedFats,transFats,proteins,alcohol)\n" + "VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?) " + "ON CONFLICT (ingredientName) DO UPDATE SET " + "price = ?," + "quantityType = ?," + "gramsPerQuantity = ?," + "complexCarbs = ?," + "simpleCarbs = ?," + "fibers = ?," + "saturatedFats = ?," + "unsaturatedFats = ?," + "transFats = ?," + "proteins = ?," + "alcohol = ?\n" + "WHERE ingredientName = ?";
    ...
    public void insertIntoMealsTable(String mealName) throws SQLException {
        ...
        try (Statement statement = connection.createStatement()) {
            statement.execute(String.format("INSERT INTO Meals (mealName) VALUES ('%s')" + "ON CONFLICT (mealName) DO NOTHING ", mealName));
        }
        connection.commit();
    }
    ...
    public void insertIntoMealCompositionsTable(Meal meal) throws SQLException {
        ...
        for (Ingredient ingredient : meal.getIngredients()) {
            try (Statement statement = connection.createStatement()) {
                String mealCompositionQuery = String.format("INSERT INTO MealCompositions (ingredientName, mealName, quantity)" + " VALUES ('%s','%s','%s') ON CONFLICT (ingredientName,mealName) DO " + " UPDATE SET quantity = %s ", ingredient.getName(), meal.getName(), String.format("%f", ingredient.getQuantity().getAmount()).replaceAll(",", "."), String.format("%f", ingredient.getQuantity().getAmount()).replaceAll(",", "."));
    ...
    public void insertIntoMealPlanTable(String userName, LocalDate beginningDay) throws SQLException {
        Connection connection = DBManager.getConnection();
        connection.setAutoCommit(false);
        String query = "INSERT INTO MealPlan (username, beginningDay) " + "VALUES (?, ?) ON CONFLICT(username) DO\n" + "UPDATE SET beginningDay = ?";
        try (PreparedStatement statement = connection.prepareStatement(query)) {
    ...
    public void insertIntoDailyProgressesTable(String username, DailyProgresses dailyProgresses) throws SQLException {
        ...
        connection.setAutoCommit(false);
        try (Statement statement = connection.createStatement()) {
            statement.execute(String.format("INSERT INTO DailyProgresses (username, burnedCalories, date) VALUES ('%s', %d, %d) " + "ON CONFLICT DO UPDATE SET " + "burnedCalories = %d," + "date = %d ", username, dailyProgresses.getBurnedCalories(), dailyProgresses.getDate().toEpochDay(), dailyProgresses.getBurnedCalories(), dailyProgresses.getDate().toEpochDay()));
        }
        connection.commit();
    }
    ...
    private void insertGroceriesIngredient(Set<Ingredient> checkList, Connection connection, String status) throws SQLException {
        for (Ingredient neededIngredient : checkList) {
            try (Statement statement = connection.createStatement()) {
                statement.execute(String.format("INSERT INTO GroceriesItems " + "(ingredientName, quantity, status) VALUES ('%s',%s,'%s')", neededIngredient.getName(), String.format("%f", neededIngredient.getQuantity().getAmount()).replaceAll(",", "."), status));
            }
        }
    }
